...
const fondo = ref(0)
// esta asignacion es mala practica pero ya se hizo una asignacion reactiva a la opacidad de los iconos y la posicion absoluta
watch(mouseOpacity.value, (actual) => {
  fondo.value = actual.fondo
})
const endWord = (str: string) => {
  const shortstr = str.split('-')
  return shortstr[shortstr.length - 1]
}
...
<template>
  <div v-cloak="true" class="v-proyects">
    <h3>{{ proyectName }}</h3>
    <figure
      @mouseleave="clieckd(mouseOpacity, { valFondo: 0, valIcons: 0 })"
      @mouseover="clieckd(mouseOpacity, { valFondo: 0.8, valIcons: 1 })"
    >
